...
class Wppr_Public
{
	 ...
	 * Check if need modal
	 */
	public function check_privacy_report()
	{
		return get_field('enable_privacy_reports', 'option') || get_field('enable_third_party_reviews', 'option');
	}

	/**
	 * Check if need privacy report
	 */
	public function check_modal()
	{
		return get_field('enable_privacy_reports', 'option');
	}
	...
	public function load_review_assets($review = null)
	{
		...
		if ($this->check_modal())
			wp_enqueue_script($this->plugin_name . '-wu-modal', WPPR_URL . '/assets/js/wu-modal.js', array(), $this->version, true);
